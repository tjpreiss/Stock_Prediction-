...
def extract_features():
    ...
    stk_tickers = ['AMZN']                          # FIX: was ['AMZN'] but body used MSFT/GOOGL/IBM
    ...
    # FIX: was ('Adj Close', 'MSFT') — changed to AMZN
    Y = np.log(stk_data.loc[:, ('Adj Close', 'AMZN')]).diff(return_period).shift(-return_period)
    Y.name = Y.name[-1] + '_Future'
    
    # FIX: was ('Adj Close', ('GOOGL', 'IBM')) — AMZN is the only stock downloaded,
    # so X1 is the lagged log-return of AMZN itself (used as an autoregressive feature)
...
def extract_features_pair():
    ...
    START_DATE = (datetime.date.today() - datetime.timedelta(days=365)).strftime("%Y-%m-%d")
    END_DATE = datetime.date.today().strftime("%Y-%m-%d")
    # FIX: was ['AAPL', 'MPWR'] — changed to AMZN/GOOGL to match chosen ticker
    stk_tickers = ['AMZN', 'GOOGL']
    
    stk_data = yf.download(stk_tickers, start=START_DATE, end=END_DATE, auto_adjust=False)

    # FIX: was 'AAPL' — changed to 'AMZN'
    Y = stk_data.loc[:, ('Adj Close', 'AMZN')]
    Y.name = 'AMZN'

    # FIX: was 'MPWR' — changed to 'GOOGL'
    X = stk_data.loc[:, ('Adj Close', 'GOOGL')]
    X.name = 'GOOGL'
